...
#num_primes = 2  # only 2 primes are suppoorted for now
#ret = subprocess.call(["./mod_prodp_gen.py"])

n=sys.argv[1]

prodfp = open('mod_prodp.json','r')
pfp = open('mod_p.json','r')
mod_p = json.load(pfp)
mod_prodp = json.load(prodfp)

np1_modp_str = ''
for p in mod_p.keys():
    np1_modp_str += ' , ' + str((int(n)+1) % int(p))
    print(np1_modp_str)

np1_modp_str = np1_modp_str[3:]  #Correctng first element
permlst = mod_prodp[np1_modp_str] # the list we are looking for
...
def check_modlist(a,b,k,index,n):

    r = a * k + b

    if (r % p_prod) in permlst:
        pos.setdefault(index, []).append(k)
        ret_mod = check_eq(r,n)
        return(ret_mod)
    return(0)

def check_pos(a,b,k,n):

    r = a * k + b

    ret_pos = check_eq(r,n)
    return(ret_pos)
...
def prep_check(s1,s2,s3,n):
    ...
    m_e = min(e)
    m_g = min(g)
    while (m_g <= (m_g + p_prod)):
        m_g += 1
        i = 0
        while (i < l):
            ret_modlist = check_modlist(a[i],b[i],g[i],i,n)
            if ret_modlist == 1 :
                return
            g[i] += 1
            if (g[i] >= m_e):
    ...
    # Complete the search using position list

    m = 2
    while (m_g <= m_e):
        for pos_i in range(0,p_prod):
            eq_i = 0
            while (eq_i < l):
                km = m * p_prod + pos[eq_i][pos_i]
                ret_pos = check_pos(a[eq_i],b[eq_i],km,n)
                if ret_pos == 1 :
                    return
                g[eq_i] += 1
                if (g[eq_i] >= m_e):
