...
def check_eq(a,b,k,n,fplot):
	...
	res = sqrt((interm))
	fplot.write(str(k) + " " + str(interm) + " " + str(res) + '\n')
	#print interm
	if (res.is_integer()):
		print "\n----------------------------------------------"
...
def prep_check(s1,s2,s3,n):
	...
	for i in range(1,l+1):
		...
		a.append(eq[s1 + s2 + '|' + s3]['a'+ str(i)])
		b.append(eq[s1 + s2 + '|' + s3]['b'+ str(i)])
...
#		print 'e' + str(i) + ' = ' + str(e[i-1])
	...
	while (k <= m_e):
		...
		i = 0
		while (i < l):
			print 'for index ' + str(i) + ' at k = ' + str(k)
			check_eq(a[i],b[i],k,n,fplot[i])
			i += 1
		if (k >= m_e):
			ind_e = e.index(m_e)
			del e[ind_e]
